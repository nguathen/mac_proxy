#!/usr/bin/env bash
# monitor_wireproxy.sh
# Monitor wireproxy connection stability
...
# Monitor loop
monitor_loop() {
    log "🔍 Starting wireproxy monitor (interval: ${CHECK_INTERVAL}s, threshold: ${FAIL_THRESHOLD})"
        ...
        for config_info in "${configs[@]}"; do
            ...
            if test_connection "$port"; then
                ...
                if [ "${fail_counts[$instance]}" -gt 0 ]; then
                    log "✅ Wireproxy $instance (port $port) recovered"
                fi
                fail_counts["$instance"]=0
            else
                ...
                log "⚠️  Wireproxy $instance (port $port) failed ($count/$FAIL_THRESHOLD)"
                
                # Check if threshold reached
                if [ "$count" -ge "$FAIL_THRESHOLD" ]; then
                    log "🔄 Restarting wireproxy $instance (port $port) - threshold reached"
                    
                    # Restart specific instance
                    local pid_file="$LOG_DIR/wireproxy${instance}.pid"
                    ...
                    # Start
                    local log_file="$LOG_DIR/wireproxy${instance}.log"
                    nohup ./wireproxy -c "$conf_file" > "$log_file" 2>&1 &
                    local new_pid=$!
                    echo "$new_pid" > "$pid_file"
                    
                    log "✅ Wireproxy $instance restarted (PID: $new_pid)"
